...
import { ApiCommunication } from "./components/base/Communication/ApiCommunication";
...
const buyerModel = new Buyer("cash", "dog@mail.ru", "35-36-37", "Milan"); //Объект - экземпляр класса Buyer
...
function checkMethods() {
  ...
  //Cохранение и получение товара для подробного отображения
...
async function loadProductsByApi() {       // функция для загрузки данных о товарах с сервера и сохранения полученного массива товаров
  try {
    ...
    if (response && Array.isArray(response.items)) {
      ...
      productsModel.products = productsArr;  //cохранение массива товаров
      console.log("Каталог товаров, полученный с сервера: ", productsModel.products);  //получение массива товаров
    }
  } catch (error) {
...
//Проверка работы классов с данными сервера
function checkMethodsAfter() {
  //Методы класса Products
  //Получение одного товара по его id
  const productById = productsModel.getProductById("6a834fb8-350a-440c-ab55-d0e9b959b6e3");
  console.log("Получение товара по его id: ", productById);
  //Cохранение и получение товара для подробного отображения
  productsModel.product = productsModel.products[6];
  console.log("Товар из каталога: ", productsModel.product);

  //Методы класса Cart
  //Добавление товара в массив корзины
  cartModel.addProductToCart(productsModel.products[9]);
  cartModel.addProductToCart(productsModel.products[8]);
  cartModel.addProductToCart(productsModel.products[5]);
  //Получение массива товаров, которые находятся в корзине
  console.log("Массив товаров из корзины: ", cartModel.cartProducts);
  //Получение стоимости всех товаров в корзине
  console.log("Стоимость всех товаров в корзине: ", cartModel.getTotalCost());
  ...
  console.log("Общее количество товаров в корзине: ", cartModel.getTotalQuantity());
  //Удаление товара из массива корзины
  cartModel.deleteCartProduct(productsModel.products[9]);
  console.log("Удаление товара из массива корзины: ", cartModel.cartProducts);
  //Проверка наличия товара в корзине по его id
  const checkId = cartModel.checkProductById("f3867296-45c7-4603-bd34-29cea3a061d5");
  console.log("Проверка наличия искомого товара в корзине: ", checkId);
  //Очистка корзины
  cartModel.clearCart();
  console.log("Очистка корзины: ", cartModel.cartProducts);
